...
MCP 服务器，通过 **125 条命令**（122 条 gdext + 3 条服务端编辑器控制）将 Godot 4.6+ 编辑器暴露给 AI 工具。
...
```
server handler → EditorControl（3 条，服务端）
gdext route_tool_call → MetaCommands(4) → NodeCommands(17) → PropertyCommands(21)
...
**CI 门禁**（`.github/workflows/ci.yml`，Ubuntu）：`cargo fmt --check --all` → `cargo clippy --workspace -- -D warnings` → `cmake -B build -S .` → `cmake --build build --config Debug` → `cargo test --workspace`（50 个测试，全部离线，无需 Godot）。

**Windows 上**：`python`/`python3` 是 Microsoft Store 存根——使用 `py -3`。

**注意事项**：
- 重建后需重启 MCP 客户端——它持有旧的 `godot-mcp-server.exe`。CMake 会自动终止；手动操作则先 `taskkill`/`pkill`。
- Godot 编辑器加载插件时 `godot_mcp_gdext.dll` 被锁定。重建前需关闭编辑器或禁用插件。
- 锁定依赖：`godot = "=0.5"`、`rmcp = "=1.7"`。未经测试不要升级。
- `Cargo.lock` 已提交（二进制 crate）。不要随意重新生成。
- `rust-toolchain.toml` 指定 `channel = "stable"`。
- `plugin.cfg` 版本通过 CMake 从 `Cargo.toml [workspace.package].version` 自动同步。只需改 cargo 版本，不要改 plugin.cfg。
- CMake 将构建的 DLL 复制到 `example/addons/godot_mcp/bin/`。`example/*` 目录已被 gitignore（例外：`project.godot`、`icon.svg`、`.gitignore`）。DLL 不被版本控制跟踪。

## 测试项目目录

测试/示例 Godot 项目位于 `example/`（从 `godot/` 重命名而来）。**但** `handler.rs::resolve_project_path` 中 `project_path` 默认值仍硬编码为 `"godot"`。这意味着不带显式 `project_path` 调用 `godot_editor_open` / `godot_editor_restart` 会失败——使用这些工具时务必传 `{"project_path": "example"}`。
...
`godot_editor_open/close/restart` 在 `server/handler.rs` 中处理——不会到达 gdext。关闭编辑器会断开 WebSocket，所以响应来自服务器进程。
...
- 所有 `cmd_*` 都是**自由函数**——闭包无法在 `dispatcher.submit()` 跨越时捕获 `&self`。错误时返回 `json!({"error": "..."})`；`pipe()` 转换为 `Result<Value, String>`。
...
- README 写 "99 commands"——实际计数为 **125**。忽略该数字。
- README 写 `rust-toolchain.toml` 固定 `1.83.0`——实际只写了 `stable`。
