...
- **所有平台使用 `uv run python`**：`uv` 确保依赖一致且自动激活虚拟环境。Windows 裸 `py -3` 在 Microsoft Store stub 下会挂。`build.py` 自动检测 `vswhere` + Ninja + MSVC `cl.exe`(build.py:77-93, 160-193)。
- **陈旧缓存自动重试**：`build.py:145-147` 检测 MSB4019/VCTargetsPath 等错误模式，自动执行 `--clean` 后重试一次(build.py:236-239)。
...
- **构建优化**(extensions/CMakeLists.txt)：
  - **sccache/ccache**：根 CMakeLists.txt:29-35 自动检测，加速增量构建 2-5x。
  - **PCH**：`extensions/CMakeLists.txt:180-194`，MSVC 启用（~30-50% 加速），GCC+ccache 时自动禁用。
  - **Unity (jumbo) build**：`extensions/CMakeLists.txt:140-170`，默认开启，batch size 自动匹配 CPU 核数（上限 32）。
  - **lld-link**：`extensions/CMakeLists.txt:200-206`，自动检测 MSVC + lld-link 加速链接。
  - **PCH 头文件**：`extensions/src/pch.hpp`（包含 godot-cpp 常用头）。
- **输出目录**：GDExtension DLL 复制到 `example/addons/godot_mcp/bin/`(CMakeLists.txt:96-102)。`plugin.cfg` 与 `godot_mcp.gdextension` 由根 CMakeLists.txt 自动生成(CMakeLists.txt:59-83)。
...
- **入口符号** `gdext_rust_init`（`extensions/src/register_types.cpp:45`）——遗留名，兼容 `.gdextension` 配置，**不要改**。
...
- **端口**：默认 9600，env `GODOT_MCP_HTTP_PORT` 覆盖（含范围校验，`editor_plugin.cpp:30-41`）。
- **端点**：`/mcp`（JSON-RPC 2.0 + SSE），`/run-tests`（YAML 测试）。
- **.opencode/opencode.json** 已预配置 MCP server。
- **双重注册**：`GDREGISTER` 注册 SDK 类 + EditorPlugin（`register_types.cpp:26-31`），`HandlerRegistry` 管理 `ITool` 主表与 SDK `CommandFn` 旁路表(`server/registry/handler_registry.cpp:79-96`)。

## 添加内置工具

**无需**改 `CMakeLists.txt` 或 `handler_registry.cpp`。CMake `GLOB` 自动编译 `extensions/src/built_in/tools/**/*.cpp` (`extensions/CMakeLists.txt:85-86`)，`tools/codegen.py` 扫描 `// @tool register` 注释生成注册代码(`extensions/CMakeLists.txt:91-116`)。
...
- **顶级分类**硬编码于 `server/registry/handler_registry.cpp:183-192` 的 `top_level_meta()`。新顶级需同步加 meta。
...
- **场景树工具**：`editor_tools/scene_tree/`（20 工具）。所有场景树修改操作使用 `EditorUndoRedoManager`（通过 `EditorInterface::get_singleton()->get_editor_undo_redo()`），不直接调用裸 `UndoRedo`。剪贴板通过 `PackedScene::pack()` / `instantiate()` 实现。
- **`built_in/tool_base.hpp:29-68`**：`ITool` 接口契约（`ToolResult`, `ToolContext`, 虚方法表）。

## SDK 层（GDScript / C# 自定义工具）

- `McpToolDefinition`（`sdk/mcp_tool_definition.hpp`）——GDScript 可继承。
- `McpToolRegistry` 单例：Mode A `register_definition(this)` 或 Mode B `register_tool(name, ...)`(`sdk/mcp_tool_registry.cpp:113+`)。**注册名自动加 `custom_` 前缀**(`sdk/mcp_tool_registry.cpp:38-42`)。
- 两条路径调 `HandlerRegistry::register_custom_tool()` → `CommandFn` 旁路表。

## C++ 注意事项

- **MSVC UTF-8**：根 `CMakeLists.txt:43` 已加 `/utf-8 /bigobj`。**含非 ASCII 的字符串字面量必须用 `String::utf8("中文")`**，否则 MSVC 按 GBK 解释。
- **常用 helper**（`built_in/cmd_utils.hpp`）：
...
- **带内 UI**：编辑器底部 "Tests" 面板（`TestRunnerDock`，`plugin/test_runner_dock.hpp`）可手工运行 YAML。
