...
- **`register_existing.hpp` 与 `register_docs.hpp` 重复注册**：`SearchDocsTool`、`GetClassInfoTool`、`GetBestPracticesTool` 在两个文件中各注册一次。`register_tool()` 按 name 覆盖，不影响运行，但添加新工具时注意不要重复。
