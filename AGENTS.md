...
uv run python main.py build --purge-cache # 仅清 _deps/（FetchContent 缓存损坏时恢复）
...
cmake --build build --target deep-clean   # 删 example/bin/ + _deps/
```
...
- **版本号**仅 `CMakeLists.txt:16` `PROJECT_VERSION`。`plugin.cfg` 和 `.gdextension` 由 `main.py build` 自动生成（`scripts/_addon.py:generate_addon_configs()`）。
...
- **Pinned deps**：`godot-cpp 10.0.0-rc1`（`extensions/CMakeLists.txt:17` via FetchContent）、`ryml v0.7.0`（`:27`）。升级前必测。
