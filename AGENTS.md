...
CMake 构建流：`extensions/gdext/` 的 C++ GDExtension + 通过 Cython `--embed` 编译 `server/entry.py` → `tools/patch_entry_c.py` 补丁 `PYTHONHOME` → 编译为独立 `.exe`。需要项目根目录下的 `.venv`（含 Cython）。
...
- **测试**：`cd server && pytest`（conftest 等不在根目录）。pytest 配置：`asyncio_mode = auto`。
...
- **Lint**：`ruff`（line-length=88, select=E/W/F/I/B/C/N），`mypy --strict`。
