...
AI 助手 ──► godot-mcp-server ──► godot_mcp_gdext
 (stdio)   (Python/Cython)  ws://127.0.0.1:9500   (GDExtension 插件)
```
...
server/
├── src/           Python MCP 服务器——MCP stdio 传输、工具注册表、WS 客户端
└── entry.pyx      Cython 入口点，编译为可执行文件
```
...
- **依赖锁定**：`godot = "=0.5"`。未经测试不要升级。
- **Rust 频道**：`stable`（由 `rust-toolchain.toml` 指定）。
- **`Cargo.lock`** 已提交（二进制 crate）。
- **版本同步**：`Cargo.toml` → `plugin.cfg` 由 CMake 自动完成。只改 cargo 版本即可。
- **新增工具**：需同时在 `server/src/godot_mcp_server/registry.py` 注册 Schema，并在 `ws_server.rs::route_tool_call` 添加路由分支。
