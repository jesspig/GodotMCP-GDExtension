...
graph LR
    ...
    subgraph GodotProc["Godot 编辑器"]
        ...
        Commands["17 个处理器分组<br/>121 个 gdext 命令"]
...
- **双进程架构** — Python/Cython MCP 服务器 + C++ GDExtension 插件（godot-cpp 10.0.0-rc1），通过本地 WebSocket 通信
- **线程安全设计** — 异步 Python 运行时配合主线程调度器，安全访问 Godot API
...
AI 助手 ──► godot-mcp-server ──► godot_mcp_gdext.dll
  (stdio)   (Python/Cython)  ws://127.0.0.1:9500   (C++ GDExtension 插件)
```
...
└── src/
    ...
    ├── editor_plugin.cpp   EditorPlugin — 通过 _on_process_frame() 轮询 WS
    ├── ipc/ws_server.cpp   WebSocket 服务器（TCPServer + WebSocketPeer）
    ├── lsp/client.cpp      Godot LSP 客户端（GDScript 验证）
    ├── protocol/ipc_types.hpp
    └── commands/           17 个处理器分组 + 工具函数
...
cd server && pytest                           # 36 项测试（离线，无需 Godot）
...
- **版本**在 `CMakeLists.txt` 中维护（`set(PROJECT_VERSION "0.1.5-dev2")`）。仅在此处修改——`plugin.cfg` 由 CMake 生成。
...
- [架构概览](.repo_wiki/overview/architecture.md) — 双进程，Python 服务器 + C++ GDExtension
- [线程模型](.repo_wiki/overview/threading-model.md) — 主线程调度器模式
