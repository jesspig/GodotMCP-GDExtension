...
class AddNodeToGroupTool : public ITool {
public:
    String name() const override { return "add_node_to_group"; }
    String category() const override { return "node"; }
    String brief() const override { return "Add a node to a group"; }
    bool needs_scene() const override { return true; }
    bool needs_node() const override { return true; }
    String description() const override {
        return "Adds a node to a named group with undo support.";
    }
...
protected:
    Dictionary execute_impl(const ToolContext &ctx) override {
        ...
        if (n->is_in_group(group)) {
            ...
            data["group"] = group;
            data["hint"] = "Already in group";
            return ToolResult::ok(data);
        }

        EditorUndoRedoManager *ur = get_undo_redo();
        if (ur) {
            ur->create_action("Add " + relative_path(ctx.root, n) + " to group " + group);
