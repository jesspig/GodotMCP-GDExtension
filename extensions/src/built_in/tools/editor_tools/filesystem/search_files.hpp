...
public:
    String name() const noexcept override { return "search_files"; }
...
protected:
    ...
    static void collect_files(godot::EditorFileSystemDirectory *dir,
        ...
        for (int64_t i = 0; i < dir->get_file_count() && out.size() < max_results; i++) {
        ...
        for (int64_t i = 0; i < dir->get_subdir_count() && out.size() < max_results; i++) {
    ...
    Dictionary execute_impl(const ToolContext &ctx) override {
        ...
        bool mode_ok = false;
        for (int64_t i = 0; i < valid_modes.size(); i++) {
        ...
        if (results.size() == 0) {
            ...
            for (int64_t i = 0; i < all_files.size() && results.size() < max_results; i++) {
