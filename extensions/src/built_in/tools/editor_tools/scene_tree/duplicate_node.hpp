...
protected:
    Dictionary execute_impl(const ToolContext &ctx) override {
        ...
        if (g_undo_manager) {
            String dup_path = relative_path(ctx.root, dup);
            UndoRecord rec;
            rec.tool_name = "duplicate_node";
            rec.forward_args = ctx.args;
            Dictionary rev;
            rev["node_path"] = dup_path;
            rec.reverse_args = rev;
            rec.timestamp = godot::Time::get_singleton()->get_unix_time_from_system();
            rec.description = String("Duplicate ") + relative_path(ctx.root, node);
