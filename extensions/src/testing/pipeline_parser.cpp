...
ParseResult parse_pipeline(const godot::Dictionary &config) {
    ...
    pipeline->on_failure = parse_fail_policy(
        pipe.get("on_failure", ""), FailPolicy::FailFast);

    if (pipe.has("before_all")) {
        pipeline->before_all = parse_chain_array(pipe["before_all"]);
    }
    if (pipe.has("after_all")) {
        pipeline->after_all = parse_chain_array(pipe["after_all"]);
    }

    if (!pipe.has("stages")) {
        return make_err("Missing required key 'pipeline.stages'", "stages");
    }
    if (pipe["stages"].get_type() != godot::Variant::ARRAY) {
        return make_err("'pipeline.stages' must be an array", "stages");
    }
    const godot::Array stages_arr = pipe["stages"];

    // Pipeline-wide unique step IDs
    ...
    for (int64_t si = 0; si < static_cast<int64_t>(stages_arr.size()); ++si) {
        if (stages_arr[si].get_type() != godot::Variant::DICTIONARY) {
            return make_err(
                godot::String("stages[") + godot::String::num_int64(si) + "] must be a dictionary",
                godot::String("stages[") + godot::String::num_int64(si) + "]");
        }
        ...
        if (stage.id.is_empty()) {
            return make_err(
                godot::String("stages[") + godot::String::num_int64(si) + "].id is required",
                godot::String("stages[") + godot::String::num_int64(si) + "].id");
        }
        stage.name = stage_dict.get("name", stage.id);

        if (stage_dict.has("before_each")) {
            stage.before_each = parse_chain_array(stage_dict["before_each"]);
        }
        if (stage_dict.has("after_each")) {
            stage.after_each = parse_chain_array(stage_dict["after_each"]);
        }
        stage.on_failure = parse_fail_policy(
            stage_dict.get("on_failure", ""), pipeline->on_failure);

        if (!stage_dict.has("steps")) {
            return make_err(
                godot::String("stage '") + stage.id + "' missing 'steps'",
                godot::String("stages[") + godot::String::num_int64(si) + "].steps");
        }
        if (stage_dict["steps"].get_type() != godot::Variant::ARRAY) {
            return make_err(
                godot::String("stage '") + stage.id + "'.steps must be an array", "");
        }
        const godot::Array steps_arr = stage_dict["steps"];

        // Pass 1: parse + ID uniqueness
        ...
        for (int64_t j = 0; j < n_steps; ++j) {
            if (steps_arr[j].get_type() != godot::Variant::DICTIONARY) {
                return make_err(
                    godot::String("step in stage '") + stage.id + "' must be a dictionary", "");
            }
            ...
            if (step.id.is_empty()) {
                return make_err(
                    godot::String("step in stage '") + stage.id + "' missing 'id'", "");
            }
            if (!all_step_ids.insert(step.id).second) {
                return make_err(
                    godot::String("Duplicate step id '") + step.id + "'", step.id);
            }

            step.tool = step_dict.get("tool", "");
            if (step.tool.is_empty()) {
                return make_err(
                    godot::String("step '") + step.id + "' missing 'tool'",
                    step.id + ".tool");
            }
            ...
            if (step_dict.has("depends_on")) {
                ...
                for (int64_t k = 0; k < nd; ++k) {
                    step.depends_on.push_back(deps[k]);
                }
            }
            ...
            step.on_failure = parse_fail_policy(
                step_dict.get("on_failure", ""), stage.on_failure);
        ...
        // Pass 2: validate depends_on refs exist in same stage
        std::set<godot::String> stage_step_ids;
        for (const auto &s : parsed_steps) {
            stage_step_ids.insert(s.id);
        }
        for (const auto &s : parsed_steps) {
            for (const auto &dep : s.depends_on) {
                if (stage_step_ids.find(dep) == stage_step_ids.end()) {
                    return make_err(
                        godot::String("step '") + s.id + godot::String("' depends_on unknown '") + dep +
                        godot::String("' in stage '") + stage.id + godot::String("'"),
                        s.id + godot::String(".depends_on"));
                }
            }
        }

        // Pass 3: cycle detection (Kahn)
        {
            ...
            if (visited != static_cast<int>(parsed_steps.size())) {
                return make_err(
                    godot::String("Cycle detected in stage '") + stage.id + "' depends_on",
                    godot::String("stages[") + godot::String::num_int64(si) + "].steps");
            }
        }
