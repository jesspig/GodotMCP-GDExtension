...
> 全部 129 个 MCP 工具的描述、参数和返回值（`tool_schemas.json`），按处理器分组共 17 组。实践中 MCP 检出约 122 个。
...
| [CollisionCommands](#collisioncommands-2) | 2 | gdext（`collision.cpp`） |
| [FindCommands](#findcommands-4) | 4 | gdext（`find.cpp`） |
| [ScriptHelpersCommands](#scripthelperscommands-3) | 3 | gdext（`script_helpers.cpp`） |
| [ProjectSettingsCommands](#projectsettingscommands-7) | 7 | gdext（`project_settings.cpp`） |
| [SceneCommands](#scenecommands-16) | 16 | gdext（`scene.cpp`） |
| [ScriptGdCommands](#scriptgdcommands-5) | 5 | gdext（`script_gd.cpp`） |
| [ScriptCsCommands](#scriptcscommands-6) | 6 | gdext（`script_cs.cpp`） |
| [SearchCommands](#searchcommands-3) | 3 | gdext（`search.cpp`） |
| [UndoCommands](#undocommands-2) | 2 | gdext（`undo.cpp`） |
| [Property3dCommands](#property3dcommands-6) | 6 | gdext（`property_3d.cpp`） |
| [EditorControlCommands（gdext）](#editorcontrolcommands-gdext-7) | 7 | gdext（`editor_control.cpp`） |
...
> **注**：`list_autoloads`、`add_autoload`、`remove_autoload`、`list_scenes` 注册在 `project_settings.cpp` 的 `register_project_settings()` 中。ScriptCsCommands 已激活，但 `csharp_build` 需要 `dotnet` CLI 在 PATH 上。
...
> `register_script_cs` 已在 `register_all_tools()` 中调用，C# 工具可用。`csharp_build` 需要 `dotnet` CLI 在 PATH 上。
