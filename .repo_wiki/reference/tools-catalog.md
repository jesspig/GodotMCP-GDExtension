...
| [ScriptCsCommands](#scriptcscommands-6) | 6 | gdext（`script_cs.cpp`）**⚠ 未注册** |
...
| [EditorControlCommands（gdext）](#editorcontrolcommands-gdext-6) | 6 | gdext（`editor_control.cpp`） |
...
> **⚠ 未注册**：`register_script_cs` 在 `handler_registry.cpp` 中已声明但未在 `register_all_tools()` 中调用。运行时不可用。
