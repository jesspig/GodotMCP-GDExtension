...
> 关于 164 个 MCP 工具如何在 GodotMCP GDExtension 插件中组织、注册、发现和执行的概念概述。
...
164 个工具并非扁平列表。它们构成了一个特异性层级结构——每一层满足不同的需求。

```mermaid
flowchart TB
    subgraph Meta["Layer 3: Meta / Discovery (9 tools)"]
        M1["get_info"]
        M2["get_categories"]
        M3["get_tools"]
        M4["find_tool"]
    ...
    subgraph Semantic["Layer 2: Semantic Specific (145 tools)"]
        SC["scene_tree (24)"]
        AN["animation (10)"]
        WS["workspace (13)"]
        FS["filesystem (12)"]
...
**已合并**：`get_tool_detail` 已合并到 `get_tools(name="...", detail=true)`。元工具数量为 9。客户端配置生成通过 `get_info(include_configs=true)` 提供。`list_settings` 的 `is_meta()=false`，仅通过分类发现可见。

### 第 2 层：语义工具 — 专用工具（145 个工具）
...
经典的 MCP `tools/list` 端点只能返回扁平列表。一次返回 164 个工具会同时压垮 LLM 上下文窗口和客户端的导航能力。
...
sequenceDiagram
    ...
    R-->>S: 9 meta tools (minimal token footprint)
...
每个工具都有一个 `is_meta()` 虚方法。元工具返回 `true`——它们无条件出现在 `tools/list` 中。非元工具（145 个语义 + 2 个兜底 + 8 个文档）返回 `false`——它们对原始的 `tools/list` 调用不可见。

发现遵循一个精心设计的链条：

```
tools/list (9 个元工具, ~3KB)
...
flowchart LR
    subgraph Data["Tool Registration Data"]
        META["register/register_meta.hpp<br/>GODOT_MCP_TOOL(GetInfo, false)<br/>GODOT_MCP_TOOL(GetCategories, false)<br/>... (9 lines)"]
        EXIST["register/register_existing.hpp<br/>GODOT_MCP_TOOL(AddNode, false)<br/>... (145 lines)"]
    ...
    subgraph Result["Result"]
        TABLE["HandlerRegistry::itool_table_<br/>164 unique_ptr<ITool> entries"]
    end
...
| 维度 | 完整清单 | 渐进式披露（当前） |
|-----------|-------------|----------------------------------|
| **tools/list token 开销** | 164 个完整 schema 约 50KB | 9 个元工具名称约 3KB |
...
**来自生产 MCP 客户端的关键发现**：Claude Code 存在已知 bug，`tools/list` 结果被缓存 1 小时（`anthropics/claude-ai-mcp#45`），并且 `notifications/tools/list_changed` 被忽略（`anthropics/claude-code#4118`）。完整的 164 工具列表会静默过期。元工具方法**本质上更健壮**——发现查询总是绕过客户端缓存层，因为每次调用都是一个独立的 `tools/call` 调用。
