...
- **方法**: `POST` / `GET` / `OPTIONS`（`GET` 用于 SSE 持久流；无 `DELETE`）
...
- **CORS**: OPTIONS 预检返回 `Access-Control-Allow-Methods: POST, GET, OPTIONS`；OPTIONS 响应暴露 `MCP-Protocol-Version`，普通 GET/POST 响应额外暴露 `Last-Event-ID`
- **速率限制**: 30 req/s（全局），超限 HTTP 429
- **认证**: 可选 Bearer token（`Authorization: Bearer <token>`），常量时间比较

## 会话模型

**无 session，无 handshake。** 每请求独立处理。`server/discover` 取代 MCP 2025 的 `initialize`，返回 capabilities + serverInfo。`initialize` 和 `notifications/initialized` 作为向后兼容端点仍受支持。
...
| 速率超限 | 拒绝 | 429 | — |
| 认证失败 | 拒绝 | 401 | — |
| HTTP 方法非 POST/GET/OPTIONS | 拒绝 | 405 | GET 仅对 SSE 流有效（需 `Accept: text/event-stream`） |
...
# GET without SSE Accept（预期 406）
curl -s -o /dev/null -w "%{http_code}" -X GET http://127.0.0.1:9600/mcp
# → 406

# GET with SSE Accept（预期 200，SSE 流建立）
curl -s -o /dev/null -w "%{http_code}" -X GET http://127.0.0.1:9600/mcp \
