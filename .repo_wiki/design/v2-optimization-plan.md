...
> ADR-016 统一记录全部 10 项子决策（含 1 项'不做什么'，见 [decisions.md](decisions.md)）。
...
| 层级 | 定义 | 包含内容 | 预计工期 |
|------|------|---------|---------|
| **P0** | 阻断性：不修复则产品不可用/不可分发 | Release 流水线修复、GameBridge 安全 | 1-2 天 |
| **P1** | 竞争力：不做则在竞品中明显落后 | 编辑器 UI、关键工具补全、WSL2、CORS/Session | 2-3 周 |
| **P2** | 差异化：建立护城河 | 安全增强、客户端模板、限流 | 1-2 周 |
...
| P0-2 | GameBridge 安全 | 决策 2 |
| P1-1 | 编辑器底部面板 | 决策 3 |
| P1-2 | 关键工具补全 | 决策 4 |
| P1-3 | WSL2 支持 | 决策 5 |
| P1-4 + P2-1 | 安全模型 | 决策 6 + 决策 7 |
| P2-2 | 客户端配置 | 决策 8 |
