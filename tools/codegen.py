...
同时支持从 YAML 属性数据库生成：
  - NodePropertyGetTool/NodePropertySetTool（节点属性）
  - NodeResourceGetTool/NodeResourceSetTool（资源属性）
  - SettingGetTool/SettingSetTool（项目设置）
...
# ── 设置数据库（Project Settings）────────────────────────────────

def load_settings_db(db_dir: str) -> dict[str, list[dict]]:
    """加载设置 YAML 数据库，返回 {category_name: [setting_dict, ...]}。"""
    if not yaml:
        print("[codegen] WARNING: pyyaml not installed, skipping settings db", file=sys.stderr)
        return {}
    if not os.path.isdir(db_dir):
        return {}
    ...
    for fname in sorted(os.listdir(db_dir)):
        ...
        categories[cat_name] = data["settings"]
    return categories


def sanitize_setting_name(name: str) -> str:
    """将设置路径转为合法的 C++ 标识符/工具名。

    "display/window/size/viewport_width.android" -> "display_window_size_viewport_width_android"
    """
    return name.replace("/", "_").replace(".", "_")


def setting_category_path(full_name: str) -> str:
    """从设置全路径提取 MCP 分类路径（去掉叶子段）。

    "display/window/size/viewport_width" -> "display/window/size"
    "display/window/size/viewport_width.android" -> "display/window/size"
    """
...
def generate_setting_registrations(settings_by_cat: dict[str, list[dict]]) -> list[str]:
    """生成项目设置工具的注册代码行。"""
    lines = []
    cat_root = "editor_tools/settings"

    # 将所有设置展平成按全路径排序的列表
    flat_settings = []
    for cat_name, setting_list in settings_by_cat.items():
        for s in setting_list:
            flat_settings.append(s)

    flat_settings.sort(key=lambda s: s["name"])

    for s in flat_settings:
        full_name = s["name"]
        sub_cat = setting_category_path(full_name)
        cat_path = f"{cat_root}/{sub_cat}" if sub_cat else cat_root
        set_name = sanitize_setting_name(full_name)

        get_name = f"get_{set_name}"
        set_name_out = f"set_{set_name}"

        # Get tool
        lines.append(f"    // {full_name} (get)")
        lines.append(f"    reg.register_tool(std::make_unique<SettingGetTool>(")
        lines.append(f'        "{get_name}", "{cat_path}", "{full_name}"));')
        lines.append("")

        # Set tool
        lines.append(f"    // {full_name} (set)")
        lines.append(f"    reg.register_tool(std::make_unique<SettingSetTool>(")
        lines.append(f'        "{set_name_out}", "{cat_path}", "{full_name}"));')
        lines.append("")
...
def generate_code(tools: list[dict], nodes: dict[str, dict] = None,
                  ...
                  settings: dict[str, list[dict]] = None) -> str:
    ...
    need_settings = settings is not None
    ...
    if need_settings:
        lines.append('#include "built_in/tools/editor_tools/settings/settings_tool.hpp"')
    ...
    has_any = tools or need_node_prop or need_node_res or need_settings
    if not has_any:
        ...
        # 注册项目设置工具
        if need_settings:
            lines.append("    // ── Project Setting Tools (auto-generated from YAML) ──")
            lines.append("")
            set_lines = generate_setting_registrations(settings)
...
def main():
    ...
    parser.add_argument(
        "--settings-db",
        default=None,
        help="Directory containing YAML project settings database",
    )
    parser.add_argument(
        "--output",
        required=True,
    ...
    settings = {}
    if args.settings_db:
        settings = load_settings_db(args.settings_db)
    code = generate_code(tools, nodes or None, resources or None, settings or None)
    ...
    set_count = len(settings) if settings else 0
    set_total = sum(len(v) for v in settings.values()) if settings else 0
    print(f"[codegen] Found {len(tools)} @tool-register tool(s), "
          f"{node_count} node type(s), {res_count} resource type(s), "
          f"{set_count} setting category(ye)s ({set_total} settings), "
          f"wrote {args.output}")
